...
import { ArrowLeft, ArrowRight, Users, IndianRupee, PlusCircle, UserPlus, List, Trash2, ShieldAlert, Mail, FileText, Tag, Send, Crown, UserMinus, BarChart3, ShieldCheck } from 'lucide-react';
import ThemeToggle from '../components/ThemeToggle';
import toast from 'react-hot-toast';

const GroupDashboard = ({ groupId, onBack }) => {
    const { user, isSystemAdmin, refreshUser } = useAuth();
    ...
    const handlePayAction = (amount, toUserId, toUserName) => {
        setConfirmModal({
            isOpen: true,
            title: 'Settle Debt',
            message: `Are you sure you want to officially record a payment of ₹${amount} to ${toUserName}?`,
            ...
            onConfirm: async () => {
                ...
                try {
                    setIsSubmitting(true);
                    await groupService.settleDebt({
                        groupId: groupId,
                        toUserId: toUserId,
                        amount: amount
                    });
                    toast.success(`Successfully settled ₹${amount} with ${toUserName}!`);
    ...
    const handleMarkPaid = async (splitId, memberId, memberName) => {
        try {
            setIsSubmitting(true);
            // groupId is needed by groupMemberMiddleware (sent in request body)
            await groupService.markSplitAsPaid(splitId, memberId, groupId);
            toast.success(`${memberName}'s share marked as paid!`);
            fetchData();
        } catch (error) {
    ...
    const handleSettleAll = (toUserId, toUserName) => {
        setConfirmModal({
            isOpen: true,
            title: 'Settle All Debts',
            message: `Are you sure you want to settle ALL outstanding splits with ${toUserName}? This will mark multiple transactions as paid.`,
            ...
            onConfirm: async () => {
                ...
                try {
                    setIsSubmitting(true);
                    const res = await groupService.settleAll(groupId, toUserId);
    ...
    const handleDeleteGroup = () => {
        setConfirmModal({
            isOpen: true,
            title: 'DANGER: Purge Group Hub',
            message: 'Are you sure you want to permanently delete this group? All shared financial data, split history, and member connections will be IRREVERSIBLY destroyed.',
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    await refreshUser(); // Remove deleted group from sidebar immediately
                    toast.success('Group has been purged successfully');
                    onBack();
                } catch (error) {
        ...
        <div className="page-content">
                    ...
                    {canManageGroup && (
                        <>
                            <button 
                                ...
                                style={{ background: 'var(--bg-card)', border: '1px solid var(--warning)', color: 'var(--warning)', padding: '0.75rem 1.25rem', borderRadius: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 800, fontSize: '0.85rem' }} 
                                onClick={() => handleInvite && setShowInvite(true)}
                            >
                                <UserPlus size={18} /> <span>Invite</span>
                            </button>
                            <button 
                                className="btn hover-lift" 
                                style={{ background: 'var(--bg-card)', border: '1px solid var(--primary)', color: 'var(--primary)', padding: '0.75rem 1.25rem', borderRadius: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 800, fontSize: '0.85rem' }} 
                                onClick={() => navigate(`/group-admin/${groupId}`)}
                                title="Group Administration Hub"
                            >
                            ...
                            {isGroupAdmin && (
                                <button 
                                    className="btn hover-lift" 
                                    style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid var(--danger)', color: 'var(--danger)', padding: '0.75rem', borderRadius: '0.75rem' }} 
                                    onClick={handleDeleteGroup} 
                                    title="Purge Group Hub"
                                >
                                    <Trash2 size={18} />
                                </button>
                            )}
                                    ...
                                    <div key={idx} className="hover-lift" style={{ 
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                                            <div style={{ textAlign: 'right' }}>
                                                ...
                                                <span style={{ fontSize: '1.25rem', fontWeight: 900, color: 'var(--warning)' }}>₹{(s.amount || 0).toLocaleString()}</span>
                                            </div>
                                            {s.fromId === user._id && !isSystemAdmin && (
                                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                                    <button
                                                        onClick={() => handleSettleAll(s.toId, s.to)}
                                                        disabled={isSubmitting}
                                                        className="btn hover-lift"
                                                        style={{ height: '2.5rem', padding: '0 1rem', fontSize: '0.75rem', fontWeight: 900, background: 'rgba(34, 211, 238, 0.1)', border: '1px solid var(--accent)', color: 'var(--accent)' }}
                                                    >
                                                        Settle All
                                                    </button>
                                                    <button
                                                        onClick={() => handlePayAction(s.amount, s.toId, s.to)}
                                                        disabled={isSubmitting}
                                                        className="btn btn-success"
                                                        style={{ height: '2.5rem', padding: '0 1rem', fontSize: '0.75rem', fontWeight: 900 }}
                                                    >
                                                        <Send size={14} /> Settle
                                                    </button>
                                                </div>
                    ...
                    <section className="card" style={{ padding: '2.5rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            {splits.length === 0 ? (
                                ...
                                splits.map((split) => {
                                    const payerId = split.payer?._id || split.payer;
                                    const payerInfo = group.members.find(m => m._id === (payerId?.toString() || '')) || split.payer || { name: 'Unknown' };
                                    const isSettlement = split.expenseId?.category === 'Settlement';
                                    
                                    return (
                                        <div key={split._id} className="ledger-row" style={{ 
                                            borderLeft: isSettlement ? '4px solid var(--accent)' : (payerId === user._id ? '4px solid var(--success)' : '4px solid var(--warning)'),
                                            background: isSettlement ? 'rgba(0,0,0,0.1)' : 'var(--bg-main)',
                                            padding: '1.25rem',
                                            borderRadius: '1rem',
                                            marginBottom: '1rem',
                                            display: 'flex',
                                            ...
                                            <div 
                                                onClick={() => payerInfo?.avatar && setSelectedAvatar(payerInfo.avatar)}
                                                style={{ width: '48px', height: '48px', borderRadius: '14px', overflow: 'hidden', background: 'var(--bg-main)', border: '1px solid var(--border)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: payerInfo?.avatar ? 'pointer' : 'default' }}
                                            >
                                                {payerInfo?.avatar ? (
                                                    <img src={payerInfo.avatar} alt={payerInfo.name || 'User'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                ) : (
                                                    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, var(--primary) 0%, var(--accent) 100%)', color: 'white', fontWeight: 900, fontSize: '1.1rem' }}>
                                                        {(payerInfo?.name || 'U').charAt(0).toUpperCase()}
                                                    </div>
                                                )}
                                            </div>
                                            <div style={{ flex: 1, marginLeft: '0.5rem' }}>
                                                <p style={{ fontWeight: 800, fontSize: '1.1rem', color: 'var(--text-primary)', letterSpacing: '-0.01em' }}>{split.expenseId?.title || 'Expense'}</p>
                                                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.25rem', fontWeight: 600 }}>
                                                    Paid by <span style={{ color: 'var(--text-primary)', fontWeight: 800 }}>{payerId === user._id ? 'YOU' : (payerInfo?.name || 'Unknown').toUpperCase()}</span> • {new Date(split.expenseId?.createdAt || new Date()).toLocaleDateString()}
                                                </p>
                                            </div>
                                            <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                                                <div className={isSettlement ? '' : (payerId === user._id ? 'amount-positive' : 'amount-negative')} style={{ fontWeight: 900, fontSize: '1.25rem', letterSpacing: '-0.02em', color: isSettlement ? 'var(--text-primary)' : undefined }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                                    <div style={{
                                                        ...
                                                        borderRadius: '0.5rem',
                                                        background: isSettlement ? 'rgba(34, 211, 238, 0.1)' : 'rgba(108, 92, 231, 0.1)',
                                                        color: isSettlement ? 'var(--accent)' : 'var(--primary-light)',
                                                        fontSize: '0.65rem',
                                                        fontWeight: 900,
                                                        letterSpacing: '0.05em',
                                                        textTransform: 'uppercase',
                                                        border: isSettlement ? '1px solid rgba(34, 211, 238, 0.2)' : '1px solid rgba(108, 92, 231, 0.2)'
                                                    }}>
                                                        {split.expenseId?.category || 'General'}
                                                    </div>
                                                    {split.owes?.length > 0 && !isSettlement && (
                                                        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                                                            {/* Individual Pay Button for the current user if they owe */}
                                                            {split.owes.some(o => o.user?._id === user._id && o.status === 'unpaid') && (
                                                                <button 
                                                                    onClick={() => handleMarkPaid(split._id, user._id, 'My')}
                                                                    className="btn hover-lift"
                                                                    style={{ padding: '0.4rem 0.8rem', background: 'var(--success)', color: 'white', border: 'none', borderRadius: '0.5rem', fontSize: '0.65rem', fontWeight: 900, cursor: 'pointer' }}
                                                                >
                                                                    PAY NOW
                                                                </button>
                                                            )}
                                                            <div style={{ display: 'flex', gap: '4px' }}>
                                                                {split.owes.map((o, idx) => (
                                                                    <div 
                                                                        key={idx} 
                                                                        title={`${o.user?.name || 'Member'}: ${o.status}`}
                                                                        onClick={() => payerId === user._id && o.status === 'unpaid' && handleMarkPaid(split._id, o.user?._id || o.user, o.user?.name)}
                                                                        style={{ 
                                                                            width: '8px', 
                                                                            height: '8px', 
                                                                            borderRadius: '50%', 
                                                                            background: o.status === 'paid' ? 'var(--success)' : 'var(--danger)',
                                                                            cursor: (payerId === user._id && o.status === 'unpaid') ? 'pointer' : 'default',
                                                                            border: (payerId === user._id && o.status === 'unpaid') ? '1px solid white' : 'none'
                                                                        }} 
                                                                    />
                                                                ))}
                                                            </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {group.members.map((member) => (
                                ...
                                onMouseOut={(e) => e.currentTarget.style.borderColor = 'transparent'}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                        <div 
                                            style={{ width: '44px', height: '44px', borderRadius: '12px', overflow: 'hidden', border: '2px solid var(--border)', background: 'var(--bg-main)', position: 'relative', cursor: member.avatar ? 'pointer' : 'default' }}
            ...
            {selectedAvatar && (
                <div 
                    ...
                    style={{ zIndex: 100000, cursor: 'zoom-out', display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}
                >
                    <img src={selectedAvatar} alt="Enlarged Profile" style={{ maxWidth: '90vw', maxHeight: '80vh', borderRadius: '1rem', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', objectFit: 'contain' }} />
                </div>
            )}
        </div>
    );
