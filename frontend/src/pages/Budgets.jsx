...
import { ArrowLeft, Save, Trash2, PieChart, IndianRupee, Tag, AlertCircle, CheckCircle2, TrendingUp, History, Info, Sparkles, Target, BarChart3 } from 'lucide-react';
                                        ...
                                        return (
                                            <div key={b.category} className="hover-lift" style={{
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                        ...
                                                        <div>
                                                            ...
                                                            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.2rem', fontWeight: 700 }}>MONTHLY AVG: ₹{b.avgSpent.toLocaleString()}</p>
                                                        </div>
                                                    </div>
                                                    <div style={{ textAlign: 'right' }}>
                                                        <div style={{ fontWeight: 900, fontSize: '1.5rem', color: statusColor, letterSpacing: '-0.02em' }}>
                                                            ₹{b.spent.toLocaleString()} <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>/ ₹{b.limit.toLocaleString()}</span>
                                                        </div>
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    <div style={{ fontSize: '0.85rem', color: b.remaining >= 0 ? 'var(--success)' : 'var(--danger)', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                                        {b.remaining >= 0 ? (
                                                            <>
                                                                <TrendingUp size={14} /> ₹{b.remaining.toLocaleString()} Left
                                                            </>
                                                        ) : (
                                                            <>
                                                                <AlertCircle size={14} /> ₹{Math.abs(b.remaining).toLocaleString()} Over Budget
                                                            </>
                                                        )}
                                                    </div>
                                                    {b._id && (
